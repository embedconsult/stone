...
int main(int argc, char **argv) {
    ...
    /* 5. Nonexistent file fails cleanly. "SELECT 1" is a literal that never
     * touches the database file, so with SQLITE_OPEN_READONLY sqlite3 defers
     * opening the file until the first page read and sqlite3_open_v2()/
     * prepare_v2()/step() all silently succeed even though the file doesn't
     * exist. Query a real table instead (as the production callers do) so
     * the missing page actually forces the open to fail. The path itself
     * must be guaranteed-unique too: a fixed literal like "/nonexistent/..."
     * can collide with an unrelated leftover file on a shared host, which
     * would make this case pass for the wrong reason (or fail to catch a
     * regression). mkstemp() a real file, then remove it, so the path is
     * unique and known-absent at query time. */
    {
        char bad_path[4096];
        snprintf(bad_path, sizeof(bad_path), "%s/nonexistent-XXXXXX", workdir);
        int fd = mkstemp(bad_path);
        if (fd < 0) {
            perror("mkstemp");
            return 2;
        }
        ...
        char *out = (char *)0x1;
        int rc = stone_fossil_query(bad_path,
            "SELECT name FROM sqlite_master WHERE type='table' AND name='ticket'", &out);
