...
enum TicketOpener {
    ...
    /// `<remote base>/ext/chat?merge_review=<uuid>` -- the served console's
    ...
    ///
    /// Confirmed against ollama-codex's `src/chat_cgi_app.cr`: the route
    /// reads `params["merge_review"]?` (not `tkt`, which is not recognized
    /// anywhere server-side) and is served under `ext/chat` (`ui.cr`'s
    /// `chat_base_path`, default `/ext/chat`).
    nonisolated static func mergeReviewURL(remoteURL: String, ticketUUID: String) -> URL? {
        ...
        comps.queryItems = [URLQueryItem(name: "merge_review", value: ticketUUID)]
        return comps.url
    }
}
